classdef kernels < matlab.mixin.SetGetExactNames 
    ...
    methods    
        ...
        function [ghat,Khat]=ident(obj)
            ...
            ghat=obj.K*((obj.sigma2*eye(obj.n)+obj.U'*obj.U*obj.K)\(obj.U'*obj.y)); % 事後平均を計算．
        end
        ...
        function SURE(obj,x0)
            ...
            ghat_theta=@(x)  obj.kernel_matrix(x)*((obj.sigma2*eye(obj.n)+(obj.U'*obj.U)*obj.kernel_matrix(x))\(obj.U'*obj.y));% パラメータxの元での推定インパルス応答
